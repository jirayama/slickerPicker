	...
	function draw_colorPicker(h) {
      ...
      for (y = 0; y <= 130; y++) {
        for (x = 0; x <= 200; x++) {
          ...
          s = Math.easeInOutSine(x, 0, 1, 200)*100;
          l = Math.easeOutCubic(x, startL, -(startL - endL), 200);
        }
        s = 0;   
        endL = Math.easeInOutSine(y, endL, -1, 130);
        startL = Math.easeOutCubic(y, startL, -1, 130);
        l = startL;
      }
    }
